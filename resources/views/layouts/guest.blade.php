            ...
            <nav class="bg-white dark:bg-gray-900 border-b border-gray-200">
                <div class="max-w-6xl mx-auto py-2 px-4 sm:px-6 lg:px-8 flex items-center justify-between">
                    <div class="text-gray-900">
                        <a href="{{ route('index') }}">Cokorda Agung</a>
                    </div>
                    @if (Route::has('login'))
                        <div class="hidden sm:flex items-center">
                            <a href="{{ route('about') }}" class="text-sm text-gray-700">About</a>
                            @auth
                                <a href="{{ route('dashboard') }}" class="ml-4 text-sm text-gray-700">Dashboard</a>
                            @else
                                <a href="{{ route('login') }}" class="ml-4 text-sm text-gray-700">
